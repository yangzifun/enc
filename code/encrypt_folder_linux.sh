...
# 定义版本号
VERSION="v2.0-Linux-folder" # 更新版本号以示修改
...
function cleanup_temp_files {
    ...
    # 注意：这里不再 unset 变量，因为它们是局部于 encrypt_single_file 函数的
    # 并且如果脚本完全退出，它们自然会被销毁。
}
...
function encrypt_single_file {
    ...
    # 检查UUID上传API响应
    if echo "$UPLOAD_RESPONSE" | jq -e 'has("status") and (.status == "success" or .status == "warning")' &> /dev/null; then
      echo "UUID 上传成功或已存在。"
