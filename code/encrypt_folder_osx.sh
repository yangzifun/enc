...
1.  **移除常量定义**：我删除了 `UPLOAD_UUID_API_URL` 变量的定义，因为它不再被使用。
2.  **简化加密函数 `encrypt_single_file`**：我从这个核心函数中移除了整个用于上传 UUID 的逻辑块。这包括构建 JSON、发送 `curl` 请求以及检查其响应的所有代码。
3.  **版本号更新（推荐）**：我将版本号从 `v2.0-macos-folder` 更新为 `v2.1-macos-folder`，以清晰地标识这次功能上的精简。
